...
const App: React.FC = () => {
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans text-slate-900">
      ...
      {/* Change Log Modal */}
      {showChangeLog && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm animate-enter" onClick={() => setShowChangeLog(false)}>
            ...
            <div className="p-8 space-y-4 max-h-[60vh] overflow-y-auto">
              {CHANGE_LOG.map((item, idx) => (
                <div key={idx} className="flex gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                  <div className="text-orange-500 font-bold text-xs uppercase whitespace-nowrap pt-1">{item.date}</div>
                  <div className="text-sm text-slate-700 font-medium">{item.change}</div>
                </div>
              ))}
            </div>
            <div className="p-6 bg-slate-50 border-t border-slate-100 flex justify-end">
              <button onClick={() => setShowChangeLog(false)} className="px-6 py-2 bg-slate-900 text-white font-bold rounded-xl text-xs">Got it</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showMentorship && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-enter" onClick={() => setShowMentorship(false)}>
            <div className="bg-white w-full max-w-md rounded-[2rem] shadow-2xl overflow-hidden relative" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-8 space-y-6">
                    <div className="space-y-4">
                        <a href="mailto:fraz.iimi@gmail.com" className="flex items-center gap-4 p-4 rounded-2xl border-2 border-slate-100 hover:border-orange-200 hover:bg-orange-50 transition-all group">
                            <div className="w-12 h-12 rounded-full bg-orange-100 text-orange-600 flex items-center justify-center text-xl group-hover:scale-110 transition-transform">📧</div>
                            <div>
                                <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Email Me</div>
                                <div className="font-bold text-slate-800">fraz.iimi@gmail.com</div>
                            </div>
                        </a>
                        <a href="https://wa.me/919560407405" target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 p-4 rounded-2xl border-2 border-slate-100 hover:border-emerald-100 hover:bg-emerald-50 transition-all group">
                            <div className="w-12 h-12 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center text-2xl group-hover:scale-110 transition-transform">💬</div>
                            <div>
      ...
      {/* Sidebar Navigation - SLATE-900 DARK THEME */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-80 bg-slate-900 border-r border-slate-800 flex flex-col transition-transform duration-300 ease-[cubic-bezier(0.25,1,0.5,1)] lg:static lg:translate-x-0 shadow-2xl lg:shadow-none ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-900 space-y-2">
             <button onClick={() => setShowChangeLog(true)} className="w-full py-2 px-4 bg-slate-800 text-slate-400 hover:text-white rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all">📜 Simulation Logs</button>
      ...
      {/* Main Content Area */}
      <div className="flex-1 flex flex-col min-w-0 h-full relative bg-slate-50/50">
        <header className="flex-shrink-0 h-16 bg-white/80 backdrop-blur-md border-b border-slate-200 px-4 sm:px-6 flex items-center justify-between z-30 sticky top-0">
          <div className="flex items-center gap-4">
            <button onClick={() => setSidebarOpen(true)} className="p-2 -ml-2 hover:bg-slate-100 rounded-lg lg:hidden transition-colors text-slate-600">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" /></svg>
            </button>
            <div className="flex flex-col">
                 <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest hidden sm:block">Current Progress</span>
                 <div className="hidden sm:flex items-center gap-3 w-48 mt-1">
                     ...
                     <span className="text-[10px] font-bold text-orange-600 w-8 text-right">{Math.round(((currentIndex + 1) / allLessons.length) * 100)}%</span>
                 </div>
            </div>
          </div>
          <div className="flex items-center gap-2">
             <button onClick={goToPrevLesson} disabled={currentIndex === 0} className="w-9 h-9 flex items-center justify-center rounded-full border border-slate-200 text-slate-500 hover:text-orange-600 hover:border-orange-200 hover:bg-orange-50 disabled:opacity-30 transition-all">←</button>
        ...
        <div id="main-content" className="flex-1 overflow-y-auto px-4 sm:px-8 py-8 custom-scrollbar scroll-smooth">
          <div key={currentLessonId} className="max-w-4xl mx-auto space-y-8 animate-enter pb-20">
            <div className="text-center space-y-4 pt-4">
              <div className="inline-block animate-float">
                <span className="px-3 py-1 bg-orange-50 text-orange-600 text-[10px] font-black uppercase tracking-widest rounded-full border border-orange-100">Level {currentLevel.id} • {currentLevel.title}</span>
              </div>
              <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-slate-900 tracking-tight leading-tight">{currentLesson.title}</h1>
              <p className="text-base sm:text-lg text-slate-500 font-medium max-w-2xl mx-auto leading-relaxed">{currentLesson.subtitle}</p>
            </div>
            ...
            {currentLesson.id !== 'capstone-simulation' && (
              <div className="flex flex-wrap items-center justify-center gap-3 max-w-2xl mx-auto sticky top-4 z-20 pointer-events-none">
                 <div className="bg-white/80 backdrop-blur-md p-1.5 rounded-2xl border border-slate-200/60 shadow-lg flex gap-2 pointer-events-auto">
                    {currentLesson.visualType === 'interactive' && (
                       <button onClick={() => setIsAnimating(!isAnimating)} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all ${isAnimating ? 'bg-orange-500 text-white shadow-md' : 'bg-white text-slate-600 hover:bg-slate-50 border border-slate-100'}`}>
                         <span className={`${isAnimating ? 'animate-spin' : ''}`}>{isAnimating ? '⚙️' : '▶️'}</span>
                         <span className="hidden sm:inline">{isAnimating ? 'Running...' : 'Simulate'}</span>
                       </button>
                    )}
                    <button onClick={() => setShowEli5(!showEli5)} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all ${showEli5 ? 'bg-amber-100 text-amber-800' : 'bg-white text-slate-600 hover:bg-slate-50 border border-slate-100'}`}>
                      <span>🧸</span><span className="hidden sm:inline">ELI5 On</span>
                    </button>
                    <button onClick={() => setShowAnalogy(!showAnalogy)} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all ${showAnalogy ? 'bg-purple-100 text-purple-800' : 'bg-white text-slate-600 hover:bg-slate-50 border border-slate-100'}`}>
            ...
            {currentLesson.id !== 'capstone-simulation' && (
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pt-4">
                 <div className="lg:col-span-2 space-y-6">
                    {showEli5 && (
                      <div className="bg-amber-50 border border-amber-100 rounded-3xl p-6 sm:p-8 shadow-sm animate-enter relative overflow-hidden">
                        ...
                        <p className="text-base text-amber-900 leading-relaxed font-bold italic">"{currentLesson.eli5}"</p>
                      </div>
                    )}
                    {showAnalogy && (
                      <div className="bg-purple-50 border border-purple-100 rounded-3xl p-6 sm:p-8 shadow-sm animate-enter relative overflow-hidden">
                        <div className="absolute -top-4 -right-4 text-8xl opacity-5 rotate-12 pointer-events-none">💡</div>
                        ...
                        <p className="text-sm text-purple-800 leading-relaxed font-medium">{currentLesson.analogy.description}</p>
                      </div>
                    )}
                      ...
                      <ul className="space-y-4">
                        {currentLesson.shortDescription.map((item, idx) => (
                          <li key={idx} className="flex gap-4 items-start group">
                            <span className="w-6 h-6 rounded-lg bg-orange-100 text-orange-600 text-[10px] font-bold flex items-center justify-center flex-shrink-0 group-hover:bg-orange-500 group-hover:text-white transition-colors border border-orange-200">{idx + 1}</span>
            ...
            <div className="pt-12 pb-8 border-t border-slate-200/60 flex flex-col sm:flex-row items-center justify-between gap-6">
               <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-2xl bg-white border border-slate-200 flex items-center justify-center text-2xl shadow-sm">✨</div>
               ...
               {currentIndex < allLessons.length - 1 ? (
                 <button onClick={goToNextLesson} className="group w-full sm:w-auto px-8 py-4 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-wide hover:bg-slate-800 transition-all shadow-xl hover:shadow-orange-500/10 flex items-center justify-center gap-2">
